                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <form action="{{ route('business.sales.update', $sale->id) }}" method="POST" id="saleForm">
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-md-2 mb-3 d-flex align-items-end">
                                    <div class="w-100 p-2 rounded text-center" style="background:rgba(102,126,234,0.08); border:1px dashed #667eea;">
                                        <div class="text-muted" style="font-size:0.65rem; text-transform:uppercase; letter-spacing:1px;">Available Stock</div>
                                        <div class="fw-bold" style="color:#667eea; font-size:1rem;" id="totalStockDisplay">—</div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="row mb-2">
                                <div class="col-md-6">
                                    <div class="d-flex justify-content-between align-items-center mb-2">
                                        ...
                                        <button type="button" class="btn btn-sm btn-light-info" onclick="addChargeRow('', 'add', '')"><i class="feather-plus me-1"></i> Custom Charge</button>
                                    </div>
                                    <div class="mb-2 d-flex gap-1 flex-wrap">
                                        <span class="text-muted fs-11 me-1 align-self-center">Quick Add:</span>
                                        <button type="button" class="btn btn-xs btn-outline-primary py-0 px-2 fs-12" onclick="addPresetCharge('Freight / Transport', 'add')">+ Freight</button>
                                        <button type="button" class="btn btn-xs btn-outline-primary py-0 px-2 fs-12" onclick="addPresetCharge('Loading / Hamali', 'add')">+ Hamali / Labour</button>
                                        <button type="button" class="btn btn-xs btn-outline-primary py-0 px-2 fs-12" onclick="addPresetCharge('Bardana / Bag Charge', 'add')">+ Bardana</button>
                                        <button type="button" class="btn btn-xs btn-outline-primary py-0 px-2 fs-12" onclick="addPresetCharge('Mandi Tax / Cess', 'add')">+ Mandi Tax</button>
                                        <button type="button" class="btn btn-xs btn-outline-danger py-0 px-2 fs-12" onclick="addPresetCharge('Quality Deduction', 'deduct')">– Deduction</button>
                                    </div>
                            ...
                            {{-- SECTION 5: TRANSPORT & DESPATCH DETAILS --}}
                            <p class="section-label mb-2"><i class="feather-truck me-1"></i> 5. Transport & Despatch Details <small class="text-muted">(Printed on Bill)</small></p>
                            <div class="row mb-3">
                                <div class="col-md-4 mb-3">
...
// ---- Charges ----
function addChargeRow(name = '', type = 'add', amount = '') {
    var i = chargeIndex++;
    var row = `<tr id="charge_row_${i}">
        <td><input type="text" name="charges[${i}][name]" class="form-control form-control-sm" placeholder="e.g. Loading" value="${name}" required></td>
        <td>
            <select name="charges[${i}][type]" class="form-select form-select-sm" onchange="recalculate()">
                ...
                <option value="deduct" ${type === 'deduct' ? 'selected' : ''}>Deduct (–)</option>
            </select>
        </td>
        <td><input type="number" step="0.01" name="charges[${i}][amount]" class="form-control form-control-sm charge-amount" placeholder="0.00" value="${amount}" required oninput="recalculate()"></td>
...
function addPresetCharge(name, type = 'add') {
    addChargeRow(name, type, '');
}

// ---- Transport Freight Calc ----
function calcFreightBalance() {
    var fare = parseFloat($('#truck_fare').val()) || 0;
    var adv  = parseFloat($('#freight_advance').val()) || 0;
    var bal  = Math.max(0, fare - adv);
    $('#freight_balance').val(bal > 0 ? bal.toFixed(2) : '0.00');
}
